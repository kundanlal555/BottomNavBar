...
class BottomNavigationDotBarItem {
  ...
  final Text title;
  final bool roundCircle;
  final NavigationIconButtonTapCallback onTap;
  const BottomNavigationDotBarItem(
      {@required this.icon, this.title, this.roundCircle, this.onTap})
      : assert(icon != null);
}
...
class _NavigationIconButton extends StatefulWidget {
  ...
  const _NavigationIconButton(this._icon, this.title, this.roundCircle,
      this._colorIcon, this._onTapInternalButton, this._onTapExternalButton,
      {Key key})
      : super(key: key);
                ...
                child: Column(
                  children: <Widget>[
                    !widget.roundCircle
                        ? SizedBox(height: 5)
                        : SizedBox(height: 2),
                    widget.roundCircle ? roundCircle() : no_roundCircle(),
                    widget.title
                  ],
                ),
              ))));
